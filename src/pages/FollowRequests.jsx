...
/**
 * FollowRequests Page
 * Displays and manages incoming follow requests
 */
export default function FollowRequests() {
    ...
    const handleAccept = async (requestId) => {
        try {
            ...
            window.dispatchEvent(new CustomEvent('followRequestsChanged'));
        } catch (err) {
    ...
    const handleDecline = async (requestId) => {
        try {
            ...
            setError(err.message || 'Failed to decline follow request');
        } finally {
            setActionLoading(null);
        }
    };
                        ...
                        {requests.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                <div className="w-20 h-20 bg-gray-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4">
                                    <Inbox size={40} className="text-gray-400 dark:text-gray-600" />
                                </div>
                                <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                                    No pending requests
                                </h2>
                                <p className="text-gray-600 dark:text-gray-400 max-w-md">
                                    When someone sends you a follow request, it will appear here for you to accept or decline.
                                </p>
                            </div>
                        )}
                    </>
